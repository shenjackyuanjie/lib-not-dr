...
class Options:
            ...
            if isinstance(a_fun, property):
                try:
                    ...
                    raise OptionNotFound(
                        f"Option {option} is not found in {self.name}"
                    ) from None
        return values

    def str_option(
        self, shrink_to_long: Optional[int] = None
    ...
    def option_with_len(
        self
    ) -> Tuple[List[Tuple[str, Any, Type]], int, int, int]:
        """
        ...
        for key, value in options.items():
            value_t = (
                type(value) if isinstance(value, type(value)) else type(value)
            )  # 判定这个类型 是不是 基本类型
    ...
    def as_markdown(self, longest: Optional[int] = None) -> str:
        ...
        while option_len + value_len + value_type_len + 16 < console_width and (
            option_len < value[1]
            ...
            if (
                option_len < value[1]
                and option_len + value_len + value_type_len + 16 < console_width
            ):
                option_len += 1
            if (
                value_len < value[2]
        ...
        for option, value, value_t in value[0]:
            cache.write(
                f"| `{option}`{' ' * (option_len - len(option))} "
    ...
    @staticmethod
    def init_option(
        options_class: Type["Options"], init_value: Optional[dict] = None
    ) -> "Options":
